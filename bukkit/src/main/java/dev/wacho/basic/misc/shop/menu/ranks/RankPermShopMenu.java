...
public class RankPermShopMenu extends Menu {

    @Override
    public String getTitle(Player player) {
        return translate("&8Lifetime Ranks ┃ 1/1");
    }
    ...
    public Map<Integer, Button> getButtons(Player player) {
        ...
        buttons.put(23, new VelnoxButton());
    ...
    private static class LifetimeRankButton extends Button {
        ...
        public ItemStack getButtonItem(Player player) {
            return new ItemBuilder(Material.EMERALD)
                    ...
                    .setLore("&7Enhance your experience with ranks", "&7which offers you extra features!", "&7Each rank contains all features from", "&7previews one!", "",
    ...
    private static class TemporalRankButton extends Button {
        ...
        public ItemStack getButtonItem(Player player) {
            return new ItemBuilder(Material.EMERALD)
                    ...
                    .setLore("&7Enhance your experience with ranks", "&7which offers you extra features!", "&7Each rank contains all features from", "&7previews one!", "",
                            "&7Purchased days will accumulate at any", "&7time you decide to purchase this item again!", "", "&eClick to view category items.").toItemStack();
        }
    ...
    private static class CosmeticButton extends Button {
        ...
        public ItemStack getButtonItem(Player player) {
            return new ItemBuilder(Material.ENDER_CHEST)
                    .setName("&aCosmetics")
                    .setLore("&7Make you look unique by purchasing", "&7a cosmetics between all the variety", "&7we have available", "", "&eClick to view category items.").toItemStack();
        }
    ...
    private static class TagsButton extends Button {
        ...
        public ItemStack getButtonItem(Player player) {
            return new ItemBuilder(Material.NAME_TAG)
                    .setName("&aChat Tags")
                    .setLore("&7Purchase a cool chat tag to show", "&7off in the public chat.", "", "&eClick to view category items.").toItemStack();
        }
    ...
    private static class BuyingButton extends Button {
        ...
        public ItemStack getButtonItem(Player player) {
            ...
            return new ItemBuilder(playerHead)
                    ...
                    .setLore("&7You are buying for yourself!", "", "&cNote: &7If you are buying for another", "&7player, both must be connected to", "&7the same server.", "",
    ...
    private static class SearchItemButton extends Button {
        ...
        public ItemStack getButtonItem(Player player) {
            return new ItemBuilder(Material.COMPASS)
                    ...
                    .setLore("&7Find items easily by typing their name.", "", "&cFunction temporarily disabled!").toItemStack();
        }
    ...
    private static class CoinsButton extends Button {
        ...
        public ItemStack getButtonItem(Player player) {
            ...
            return new ItemBuilder(Material.GOLD_NUGGET)
                    ...
                    .setLore("&7V-Coins are used to acquire server", "&7ranks, in-game cosmetics like chat", "&7tags, suits, our particles!", "", "&7You can purchase V-Coins at",
            ...
            if (clickType == ClickType.LEFT) {
                if (currentRankIndex >= targetRankIndex) {
                    ...
                    Bukkit.dispatchCommand(Bukkit.getConsoleSender(), 
                            "alert " + playerData.getHighestRank().getPrefix() + player.getName() +
                                    " &fin our in-game store! If you want to support the server please consider visiting &c&nstore.velnox.club&f!");
    ...
    @AllArgsConstructor
    public static class VelnoxButton extends Button {

        @Override
        public ItemStack getButtonItem(Player player) {
            return new ItemBuilder(Material.LEATHER_CHESTPLATE)
                    .setRGBColor(139, 0, 0)
                    .setName("&4Velnox Rank")
                    .setLore(getsetLore(player)).toItemStack();
        }
        ...
        public void clicked(Player player, ClickType clickType) {
            ...
            if (clickType == ClickType.LEFT) {
                if (currentRankIndex >= targetRankIndex) {
                    player.playSound(player.getLocation(), Sound.ANVIL_LAND, 40F, 30L);
                } else if (playerData.getCoins() < 3500) {
                    player.playSound(player.getLocation(), Sound.ANVIL_LAND, 40F, 30L);
                } else {
                    Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "setrank " + player.getName() + " Velnox perm global Bought");
                    Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "alert " + playerData.getHighestRank().getPrefix() + player.getName() +
                            " &fin our in-game store! If you want to support the server please consider visiting &c&nstore.velnox.club&f!");
                    playerData.setCoins(playerData.getCoins() - 3500);
                    playerData.save();
                    new RankPermShopMenu().openMenu(player);
                }
            } else if (clickType == ClickType.RIGHT) {
                player.playSound(player.getLocation(), Sound.CLICK, 1F, 1F);
                new RankVelnoxMenu().openMenu(player);
            }
        }
    }

    @AllArgsConstructor
    public static class VelnoxPlusButton extends Button {
