package dev.wacho.basic.misc.cosmetics.death.killeffect.impl;

import dev.wacho.basic.Basic;
import dev.wacho.basic.misc.cosmetics.death.killeffect.EffectCallable;
import dev.wacho.basic.utils.effects.ParticleEffect;
...
public enum KillEffects {
    ...
    NUKE("Nuke", (loc) -> {
        ...
        }).runTaskTimerAsynchronously(Basic.getInst(), 0L, 1L);
    }, 10000);
    ...
    public static KillEffects getByName(String input) {
        return (KillEffects) Arrays.stream(values()).filter((type) -> {
    ...
    private KillEffects(String name, EffectCallable callable, int price) {
        this.name = name;
