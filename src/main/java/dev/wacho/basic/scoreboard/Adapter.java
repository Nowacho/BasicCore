package dev.wacho.basic.scoreboard;
    ...
    public List<String> getLines(Player player) {
        ...
        if (!aquaData.isInStaffMode()) {
            for (String string : Basic.getInst().getScoreboardFile().getStringList("IN-SPAWN")) {
                ...
                if (string.contains("%new-line%")) {
                    scores.add(string.replace("%new-line%", ""));
                }
        ...
        List<? extends Player> players = Bukkit.getOnlinePlayers().stream().filter(p -> p.hasPermission("basic.staffmode.staff")).collect(Collectors.toList());
        if (aquaData.isInStaffMode()) {
        ...
        if (!scores.isEmpty()) {
            if (Basic.getInst().getScoreboardFile().getBoolean("BARS-ENABLED")) {
                scores.add(0, "&b" + Basic.getInst().getScoreboardFile().getString("BARS"));
            }
        ...
        scores.forEach(string -> ChatColor.translateAlternateColorCodes('&', string));
